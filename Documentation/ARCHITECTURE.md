...
Builds `URLRequest`s from `Parameters`, in two forms: `urlRequest(for:parameters:)` builds `GET` requests with URL-encoded query parameters, replicating the wire behavior the library relied on before the v3.0.0 Alamofire removal (`Bool` values numerically encoded as `"1"`/`"0"`, keys sorted for determinism); `httpBodyRequest(for:parameters:)`, added for the write/action endpoints, builds `POST` requests with the same parameters URL-encoded into the HTTP body instead. As a defensive hedge for `httpBodyRequest` — based on historical Objective-C precedent suggesting Untappd's real API may expect credentials in the query string even on `POST` requests, still not live-verified — `access_token`/`client_id`/`client_secret` are additionally mirrored into the query string alongside the body; other parameters (which may include free-text user content or location data) stay body-only.
...
- Pass a `CDUntappdDecoderConfiguration` to customize the `JSONDecoder.KeyDecodingStrategy`/`DateDecodingStrategy` used to decode every response (default `.default`, matching a plain `JSONDecoder()`). Every model's explicit snake_case `CodingKeys` already handle Untappd's real response shapes without this, so it exists purely to close the configuration-surface gap with retry/monitor/adapter/cache — not because any current model needs it.
